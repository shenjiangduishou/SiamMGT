...
#stride 表示特征图相对于输入图像的下采样率。在卷积神经网络中，通过一系列的卷积和池化操作，输入图像的尺寸会逐渐减小，形成特征图。stride 就是描述这种尺寸减小的比例。例如，如果 stride = 8，意味着特征图上的一个像素对应输入图像上的 8x8 个像素区域。
#在目标检测和跟踪中，当我们有输入图像上的边界框 bbox 时，需要将其坐标转换到特征图上，以便在特征图上进行后续的操作，如生成掩码（mask）。这个转换过程就需要用到 stride
#offset 是一个偏移量，用于调整边界框坐标在特征图上的映射。它通常是为了补偿卷积和池化操作中的一些边界效应或者特定的网络结构设计。在某些情况下，网络的第一层卷积可能会有一些特殊的处理，导致特征图的起始位置相对于输入图像有一个偏移，offset 就是用来修正这个偏移的
            roi = ((bbox + 1 - offset + stride / 2) / stride - 1).int()
# bbox + 1：在某些情况下，为了避免边界框的坐标为 0 带来的问题，会给边界框的坐标加1,减去 offset 是为了修正前面提到的特征图相对于输入图像的偏移,
# 加上 stride / 2 是为了将原图边界框的坐标转换为特征图网格中心的坐标。这样做可以使坐标映射更加准确
#(bbox + 1 - offset + stride / 2) / stride：将修正后的坐标除以 stride，将输入图像上的坐标转换为特征图上的坐标
#(bbox + 1 - offset + stride / 2) / stride - 1：最后减去 1 是为了将坐标调整到特征图的索引范围内（通常特征图的索引从 0 开始）
#.int()：将计算结果转换为整数类型，因为特征图的索引必须是整数。

#这里可以试验一下在每个特征图后面进行x *= mask操作
