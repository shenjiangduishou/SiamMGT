...
from pysot.models.mgrm_sstp.sstp import Self_similar_Soft_Threshold_Processing
...
class Inception3(nn.Module):
    ...
    def __init__(self, transform_input=False):
        ...
        self.sstp = Self_similar_Soft_Threshold_Processing(channel=768, reduction=4,topk=128)
    ...
    def forward(self, x, bbox=torch.tensor([])):
        ...
        x = self.Mixed_5c(x)
        # N x 288 x 35 x 35
        x = self.Mixed_5d(x)#N*288*28*28
        # N x 288 x 35 x 35
        x = self.Mixed_6a(x)#N*768*13*13
        # N x 768 x 17 x 17
        x = self.Mixed_6b(x)
        # N x 768 x 17 x 17
        x = self.Mixed_6c(x)
