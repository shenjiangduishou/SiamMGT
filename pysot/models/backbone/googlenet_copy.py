...
from pysot.models.hspa.hspa3 import TrackingHSPA
from pysot.models.hspa.ar1 import DepthwiseSeparableConvWithWTConv2d


class Inception3_copy(nn.Module):
    r"""
    GoogLeNet
    ...
    def __init__(self, transform_input=False):
        super(Inception3_copy, self).__init__()
        ...
        self.hspa_z = TrackingHSPA(768,reduction=4, res_scale=0.2)
        self.hspa_x = TrackingHSPA(768,reduction=4, res_scale=0.2)
        # self.wt = DepthwiseSeparableConvWithWTConv2d(768,256,3,1,ex_downsampl=False)
        # self.wt_x = DepthwiseSeparableConvWithWTConv2d(768,256,3,1,ex_downsampl=False)
        # self.downsample1 = FeatureDownsample(256) #HW从33到25
        # self.downsample2 = FeatureDownsample(768)
        ...
        self.channel_reduce_z = nn.Sequential(
            nn.Conv2d(768, 256, 1),
            nn.BatchNorm2d(256, eps=0.001),
        )
        self.channel_reduce_x = nn.Sequential(
            nn.Conv2d(768, 256, 1),
            nn.BatchNorm2d(256, eps=0.001),
        )
    ...
    def forward(self, x, bbox=torch.tensor([])):
        ...
        if len(bbox):
            ...
            for i in range(x.shape[0]):
                mask[i, :, max(0, roi[i][1]): (min(roi[i][3], 12)) + 1,
                max(0, roi[i][0]): (min(roi[i][2], 12) + 1)] = 1
            x *= mask
            # wt_z = self.wt(x)
            x = self.channel_reduce_z(x)
            # return x,wt_z
            return x
        else:
            ...
            x = self.hspa_x(x)
            # wt_x = self.wt(x)
            # wt_x = self.downsample1(wt_x)
            # x = self.downsample2(x)
            # x * self.se(x)
            x = self.channel_reduce_x(x)
            # wt_x = wt_x[:, :, crop_pad:-crop_pad, crop_pad:-crop_pad]
            # return x,wt_x
            return x
