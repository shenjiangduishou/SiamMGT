...
# 论文题目：High-Similarity-Pass Attention for Single Image Super-Resolution# 中文题目：高相似度-通过注意力机制的单图像超分辨率
# 论文链接：https://arxiv.org/pdf/2305.15768# 官方github：https://github.com/laoyangui/HSPAN
# 所属机构：福州大学计算机与数据科学学院等# 代码整理：微信公众号：AI缝合术
...
class HSPA(nn.Module):
    ...
    def forward(self,input):
        ...
        return (self.res_scale*x_final.permute(0,2,1).view(N,-1,H,W)+input).contiguous()
...
def tau_support(s, dim=2, topk=128):
    ...
    tau /= support_size.to(s.dtype)#累加起来的值取平均
...
#模块参数
    batch_size =1# 批大小
    channels =768# 输入特征通道数
    height =32#图像高度
    width = 32  # 图像宽度width =32# 图像宽度
# 创建 hspa 模块
    hspa = HSPA(channel=768, reduction=2, res_scale=1,conv=default_conv, topk=128)
    print(hspa)
...
##cumsum用法示例
    ...
    # 按行方向（dim=0）进行累积和计算
    cumsum_0 = k.cumsum(dim=0)
    print("\n按行方向（dim=0）的累积和:")
    print(cumsum_0)

    # 按列方向（dim=1）进行累积和计算
    cumsum_1 = k.cumsum(dim=1)
    print("\n按列方向（dim=1）的累积和:")
    print(cumsum_1)