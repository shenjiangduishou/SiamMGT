...
class SoftThresholdingOperationFun(Function):
    ...
    def backward(cls, ctx, grad_output):
        ...
        dim = ctx.dim
        grad_input = grad_output * (output != 0).to(grad_output.dtype)
...
class Self_similar_Soft_Threshold_Processing(nn.Module):
    """适配跟踪任务的改进版HSPA模块"""

    def __init__(self, channel=256, reduction=4,topk=128):
        super(Self_similar_Soft_Threshold_Processing, self).__init__()
    ...
    def forward(self, input):
        ...
        # 动态阈值处理
        similarity = softThresholdingOperation(similarity, dim=2, topk=self.topk)
...
# 创建 mgrm_sstp 模块
    sstp = Self_similar_Soft_Threshold_Processing(channel=256, reduction=2)
    print(sstp)
