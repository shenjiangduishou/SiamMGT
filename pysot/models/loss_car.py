...
##########################################points in the gt_bbox area################################################
        all_s1 = reg_targets_per_im[:, :, 0] > 0#保留在bboxes内的点
...
##########################################ignore labels################################################
        ignore_s1 = reg_targets_per_im[:, :, 0] > 0.2 * ((bboxes[:, 2] - bboxes[:, 0]) / 2).float()#如果l的值大于label中心到边界真实距离的0.2倍则保留下来记为ignore_s1（忽略标签）
        ignore_s2 = reg_targets_per_im[:, :, 2] > 0.2 * ((bboxes[:, 2] - bboxes[:, 0]) / 2).float()#也就是保留真实左边界向右0.2倍中心到左边界距离的所有横坐标（对于l来说，其他类似）
        ...
        # 最终是将0.2-0.5倍中心到左（右、上和下）边界距离的所有横坐标设为无法判断的样本，0.5倍以内的是正样本(label中对应索引为-1和1)
        #reg_targets_per_im返回网格内（和特征图25*25大小一致）的点到边界框的值，pos_area返回特征图上bbox的真实标签所框出的所有点（包括前景和背景）
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, labels, reg_targets, neg):
        ...
        label_cls, reg_targets, pos_area = self.prepare_targets(locations, labels, reg_targets, neg)#包含正负样本标签索引的label，网格所有点到边界距离的reg_targets（76，625,4），特征图上对应bbox的所有点pos_area（框内为1，其余0。（76,625））
        box_regression_flatten = (box_regression.permute(0, 2, 3, 1).contiguous().view(-1, 4))#将预测box_regression展平方便后续操作
        labels_flatten = (label_cls.view(-1))
        reg_targets_flatten = (reg_targets.view(-1, 4))
        centerness_flatten = (centerness.view(-1))#将预测的中心值展平

        pos_inds = torch.nonzero(labels_flatten > 0).squeeze(1)#获取正样本的索引（也就是特征图上bbox中心距离到边界距离0.5倍范围内的）

        ###########################change cen and reg area###################################
        pos_area_flatten = (pos_area.view(-1))
        all_pos_idx = torch.nonzero(pos_area_flatten > 0).squeeze(1)#获取bbox所有点的索引（标为1的部分）
