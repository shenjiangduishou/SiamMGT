...
class SiamGATTracker(SiameseTracker):
    ...
    def track(self, img):
        ...
        w_z = self.size[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
        h_z = self.size[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
        s_z = np.sqrt(w_z * h_z) #将模板图片适当放大(考虑上下文信息后的区域边长)
        self.scale_z = cfg.TRACK.EXEMPLAR_SIZE / s_z #考虑上下文信息后的目标尺度到模板图像大小的缩放比例
        s_x = s_z * (cfg.TRACK.INSTANCE_SIZE / cfg.TRACK.EXEMPLAR_SIZE) #将搜索图片按比例放大
